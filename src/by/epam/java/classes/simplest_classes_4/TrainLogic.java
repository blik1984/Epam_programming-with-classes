...
public class TrainLogic {
	...
	public static void viewConcreteTrain (List<Train> trains) {
		...
		while (flag == false) {
			...
			String s1 = "ƒл€ вывода информации доступны поезда со следующими номерами: ";
			String s2 = TrainLogic.TrainList(trains);
			String s3 = "¬ыберите поезд дл€ вывода информации";
			int numberFromUser;
			int numberTrain;
	...
	/*
	 * ¬ методе sortDestinationAndTime критически важно что сортировка по времени идЄт перед сортировкой по пункту назначени€. 
	 * “аким образом поезда с одинаковым пунктом назначени€ автоматически сто€т отсортированными по времени
	 */
